...
一个 Obsidian 插件。你在某个文件夹里工作时，一键就能只留下这个文件夹里的笔记，
文件列表中的其余内容暂时被遮住，再点一下就回来。

它还能让你直接拖动文件夹来排顺序，不必再靠 `01_收件箱`、`02_项目` 这样的命名去
凑出想要的次序。
...
- **会上投屏。** 开会讲某个项目时把屏幕投出去，满屋子人看到的只有你要讲的那个
  文件夹，而不是隔壁项目、另一个客户、以及你自己私下记的那些东西的名字。投屏是
  实时的，没有剪掉重来的机会。
...
![打开调整文件夹顺序模式：折叠箭头变成手柄，下方的文件全程保持变暗且没有手柄；每拖动一个文件夹都会显示它将落在哪里](docs/arrange.gif)
